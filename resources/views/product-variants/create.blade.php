<form id="form-create-product-variant" action="{{ route('product-variants.store') }}" method="POST"
    ...
    <div class="tw-flex tw-items-center tw-justify-between tw-px-6 tw-py-4 tw-border-b tw-border-gray-100 tw-bg-white">
        <div>
            <h3 class="tw-text-lg tw-font-semibold tw-text-gray-900 tw-tracking-tight">{{ __('product_variant.create_title') }}</h3>
            <p class="tw-text-sm tw-text-gray-500 tw-mt-0.5">{{ __('product_variant.create_subtitle') }}</p>
        </div>
        ...
        <div>
            <button type="submit" id="submit-create-product-variant"
                ...
                {{ __('product_variant.save_create') }}
            </button>
        </div>
    </div>
    ...
    <div class="tw-px-6 tw-py-5 tw-bg-white tw-overflow-y-auto tw-flex-1 tw-space-y-5">
        ...
        <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-4">
            <div>
                ...
                <input type="text" name="sku" id="sku" required placeholder="{{ __('product_variant.sku_placeholder') }}"
                    class="tw-w-full tw-rounded-md tw-border-gray-300 tw-shadow-sm focus:tw-border-[#0078D4] focus:tw-ring-[#0078D4] tw-text-sm tw-px-3 tw-py-2 tw-transition-colors tw-outline-none">
            </div>

            <div>
                <label for="barcode" class="tw-block tw-text-sm tw-font-medium tw-text-gray-700 tw-mb-1.5">{{ __('product_variant.barcode') }}</label>
                <input type="text" name="barcode" id="barcode" placeholder="{{ __('product_variant.barcode_placeholder') }}"
                    class="tw-w-full tw-rounded-md tw-border-gray-300 tw-shadow-sm focus:tw-border-[#0078D4] focus:tw-ring-[#0078D4] tw-text-sm tw-px-3 tw-py-2 tw-transition-colors tw-outline-none">
            </div>
        </div>
