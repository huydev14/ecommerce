<form id="form-create-tax" action="{{ route('taxes.store') }}" method="POST"
    ...
    <div class="tw-flex tw-items-center tw-justify-between tw-px-6 tw-py-4 tw-border-b tw-border-gray-100 tw-bg-white">
        <div>
            <h3 class="tw-text-lg tw-font-semibold tw-text-gray-900 tw-tracking-tight">{{ __('tax.create_title') }}</h3>
            <p class="tw-text-sm tw-text-gray-500 tw-mt-0.5">{{ __('tax.create_subtitle') }}</p>
        </div>

        <div>
            <button type="submit" id="submit-create-tax"
    ...
    <div class="tw-px-6 tw-py-5 tw-bg-white tw-overflow-y-auto tw-flex-1 tw-space-y-5">
        <div>
            ...
            <input type="text" name="name" id="name" required placeholder="{{ __('tax.name_placeholder') }}"
        ...
        <div>
            ...
            <input type="number" min="0" max="100" step="0.01" name="rate" id="rate" required value="0"
                placeholder="{{ __('tax.rate_placeholder') }}"
                class="tw-w-full tw-rounded-md tw-border-gray-300 tw-shadow-sm focus:tw-border-[#0078D4] focus:tw-ring-[#0078D4] tw-text-sm tw-px-3 tw-py-2 tw-transition-colors tw-outline-none">
        </div>
    </div>
</form>
